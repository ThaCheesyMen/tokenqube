-- =============================================
-- NUCLEAR FIX - DROPS ALL TRIGGERS AND RECREATES EVERYTHING
-- Run this in Supabase SQL Editor
-- =============================================

-- Step 1: Drop ALL triggers that might be interfering
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        WHERE trigger_schema = 'public' 
          AND (event_object_table = 'daily_login_rewards' 
               OR trigger_name LIKE '%referral%'
               OR trigger_name LIKE '%daily_login%')
    ) LOOP
        EXECUTE 'DROP TRIGGER IF EXISTS ' || quote_ident(r.trigger_name) || 
                ' ON ' || quote_ident(r.event_object_table) || ' CASCADE';
        RAISE NOTICE 'Dropped trigger: %.%', r.event_object_table, r.trigger_name;
    END LOOP;
END $$;

-- Step 2: Drop and recreate check_daily_login function
...
-- Step 3: Create other missing functions
...
GRANT EXECUTE ON FUNCTION check_daily_login(UUID) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION update_user_heartbeat(UUID) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION set_user_offline(UUID) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_platform_stats() TO authenticated, anon;
...
BEGIN
    ...
    RAISE NOTICE 'Triggers on daily_login_rewards: %', trigger_count;
    IF trigger_count > 0 THEN
        RAISE NOTICE '⚠️  Some triggers remain - check them manually';
    ELSE
        RAISE NOTICE '✅ All interfering triggers removed!';
    END IF;
    RAISE NOTICE '==========================================';
END $$;
